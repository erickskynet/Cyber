...
class LINE extends LineAPI {
    ...
    setState(seq) {
        if(isAdminOrBot(seq.from)){
            ...
            this._sendMessage(seq,`Kamu Bukan Admin Saya`);
        }
    }
    ...
    async textMessage(txt, seq) {
        ...
        if(txt == 'batal' && this.stateStatus.cancel == 1) {
            this.cancelAll(seq.to);
        }

        if(txt == 'bot' || txt == 'cyber') {
            this._sendMessage(seq, 'Cyber Bots in hare :)');
        }
        ...
        if(txt === 'boom' && this.stateStatus.kick == 1) {
